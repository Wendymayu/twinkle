...
| 文件 | 说明 |
|---|---|
| [`twinkle/e2a/models.py`](../twinkle/e2a/models.py) | E2AEnvelope（7 字段）+ E2AResponse（8 字段）（Pydantic BaseModel） |
| [`twinkle/e2a/__init__.py`](../twinkle/e2a/__init__.py) | 导出 E2AEnvelope、E2AResponse、E2A_PROTOCOL_VERSION |

### 4.2 保留了什么

Twinkle 的 E2AEnvelope 只保留 **7 个核心字段**（对比 jiuwenclaw 的 20+ 字段）：
...
class E2AEnvelope(BaseModel):
    ...
    timestamp: float = 0.0               # 请求时刻（当前未启用，恒 0.0）
