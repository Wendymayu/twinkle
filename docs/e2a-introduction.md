...
| 文件 | 说明 |
|---|---|
| [`twinkle/e2a/models.py`](../twinkle/e2a/models.py) | E2AEnvelope + E2AResponse（Pydantic BaseModel，7 个字段） |
| [`twinkle/e2a/__init__.py`](../twinkle/e2a/__init__.py) | 导出 E2AEnvelope、E2AResponse、E2A_PROTOCOL_VERSION |

### 4.2 保留了什么

Twinkle 的 E2AEnvelope 只保留 **7 个字段**（对比 jiuwenclaw 的 20+ 字段）：
...
```
 ...
 ⑧ AgentClient._recv_loop → 按 request_id demux 投递到 asyncio.Queue
                        │
                        ▼
 ⑨ MessageHandler 扇出 → Message(event_type=chat.delta, content="你")
                        │
                        ▼
 ⑩ ChannelManager → WebChannel 广播 → 浏览器收
...
- `send_request`：为指定 request_id 创建 Queue，发请求，`wait_for` 一个帧
