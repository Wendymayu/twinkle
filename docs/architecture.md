...
- `e2a.ask` → `Message(event_type=approval.ask, payload=body)`（审批请求 `{approval_id, tool, args, ...}`，Phase 4）→ `enqueue_outbound(msg)`
- `e2a.result` → `Message(event_type=result, payload=body)`（单帧 RPC 结果 / 审批 ack）→ `enqueue_outbound(msg)`
...
### 4.9 Permissions — 工具权限 / 审批切面（Phase 4）

[twinkle/agentserver/permissions/](../twinkle/agentserver/permissions/) 是 agentserver 的 in-tree 权限切面，在 `before_tool_call` 钩子点拦截工具调用，按档位策略决定 ALLOW / ASK / DENY，ASK 挂起 run_stream 等人类审批。**业务代码零插桩调用**——通过 `PermissionHook`（[hooks/builtin/permission_hook.py](../twinkle/agentserver/hooks/builtin/permission_hook.py)）接入钩子系统（[`hooks/`](../twinkle/agentserver/hooks/) 包，`AgentHook` + `HookContext` + `HookInterrupt` + `@hook` 装饰器），依赖方向单向 `permissions → hooks → agentserver`，agentloop 只在 `except HookInterrupt` 处感知它。opt-in：`TWINKLE_PERMISSIONS` 单 JSON env（对齐 OTEL 哲学），`enabled=false` 默认 = 系统关（全 ALLOW、无审计、无 ASK；command_exec 仍走 `builtin_rules` 做 defense-in-depth）。设计与实施记录在 [spec](../superpowers/specs/2026-07-24-phase4-permissions-design.md) 与 [plan](../superpowers/plans/2026-07-24-phase4-permissions.md)。

**包内文件**（纯数据 + 纯逻辑，零 ws 依赖）：

| 文件 | 职责 |
|---|---|
| `models.py` | `PermissionLevel`(ALLOW/ASK/DENY) + `PermissionDecision`(level/reason/source/rule_id/deny_message) + `ToolPermissionLogEntry`(tool/decision/source/rule_id/reason/user_decision/channel/session_id/request_id/ts) |
| `builtin_rules.py` | command_exec 的 DENY 规则**单一真源**：`COMMAND_DENY_PATTERNS` = 8 条 Windows-aware 原有 + 9 条 verbatim 移植自 jiuwenswarm `builtin_rules.yaml`；`matches(cmd)` 命中即返 reason。command_exec 与 PermissionPolicy 共读此表，杜绝双份维护 |
| `policy.py` | `PermissionPolicy`：档位表 `tools[name]→level` + 用户 rules + `allow_always` override（运行时文件，mtime 缓存热重载）+ `global_default`。`check(tool,args)` 按合并序：① override → ② DENY（command_exec 走 builtin_rules，用户 rules 走正则）→ ③ 工具档位（`require-approval` 归一为 ASK）→ ④ global_default。`persist_allow_always()` 写 override 文件（command_exec 存 `head *` 模式，refuse bless 含 shell 元字符的链式命令） |
| `audit.py` | `ToolPermissionLog`：结构化 JSONL 审计（每次 check 写一行；ASK 流产生 2 行）。fail-soft——写失败只 log 不抛（审计不阻断主流程） |
| `approval_registry.py` | `ApprovalRegistry`：`approval_id → asyncio.Future` 单例（模块级 `APPROVAL_REGISTRY`）。`register(id)` 造 Future 供 agent_loop await 挂起；`handle_respond(envelope, send)` resolve Future + 回 `e2a.result` ack（R2）；`cancel_all()` 断连时清理 |
| `engine.py` | `PermissionEngine`：通道门（`enabled=false` 或 channel 不在 `enabled_channels` → ALLOW 透传不审计）+ 委托 `policy.check` + 写 `ToolPermissionLog` + 委托 `persist_allow_always` |
| `permission_context.py` | `APPROVAL_CHANNEL` ContextVar（照抄 `plan_todo_context.py` 形态），由 `AgentLoop._inner_run_stream` 入口 `set_permission_channel(envelope.channel)` 设定，使无参的 PermissionHook 回调能定位当前通道 |

**PermissionHook**（[hooks/builtin/permission_hook.py](../twinkle/agentserver/hooks/builtin/permission_hook.py)，`priority=100` 先于其他 before_tool_call hook）在 `before_tool_call` 按 `engine.check()` 的 `PermissionDecision.level` 分派：

- **ALLOW** → no-op，工具正常执行
- **DENY** → `ctx.request_force_finish(deny_message)`；`@hook` 装饰器短路，deny 消息作为 tool_result 回灌 agent
- **ASK** → `raise HookInterrupt(data={approval_id, tool, args, tool_call_id, reason, ...})`；已批准的 `tool_call_id` 走 bypass（`ctx.extra["_approved_tool_call_ids"]`），避免恢复后重调再问

**ASK 挂起/恢复流**（核心不变式：挂起的 run_stream 在**原 request_id** R 上恢复，审批响应用独立 request_id R2）：

```
agent_loop._inner_run_stream
  ├─ tool_call → @hook _raided_tool_call → before_tool_call(PermissionHook)
  │     └─ ASK → raise HookInterrupt(approval_id, ...)
  ├─ except HookInterrupt as hi:
  │     ├─ approval_id = hi.data["approval_id"]
  │     ├─ future = APPROVAL_REGISTRY.register(approval_id)     # 进程内 Future
  │     ├─ yield E2AResponse(response_kind="e2a.ask",           # is_final=false
  │     │                  body={approval_id, tool, args, ...})  # → gateway → approval.ask event
  │     └─ decision = await future                              # SUSPEND（不占 ws 读循环）
  │           ├─ "allow"/"allow_always" → 记 tool_call_id bypass + (allow_always 持久化) → 重执行工具
  │           └─ else → result = "[tool denied by user: ...]"   # deny 消息作 tool_result
  │     → store.append(tool_result) → continue ReAct（再查模型）
  │
  └─ approval.respond（独立 request_id R2，ws_handler 内联路由，在 active-run guard 之前）
        → APPROVAL_REGISTRY.handle_respond(envelope, send)
        → resolve(approval_id, decision) → set_result(decision)  # 唤醒挂起的 future
        → 回 e2a.result ack（R2，is_final=true）                   # 告浏览器审批已受理
```

`approval_id`（uuid）是 Future 的 key（不是 request_id），使 R2 的 `approval.respond` 能找到挂起的原 R 流——`request_id` 仍是 load-bearing 的流式分片标识，`approval_id` 是审批专用的关联键。完整时序图见 spec §4.1。

**ws_handler 并发模型**（[server.py](../twinkle/agentserver/server.py) `ws_handler(loop, store)`）：每个 chat.send 请求 `asyncio.create_task(run_task(envelope))` 起独立 task，使一个挂起等审批的 run_stream **不阻塞读下一条入站消息**（approval.respond）。`send_lock`（`asyncio.Lock`）串行化 ws 写。**每会话一活跃 run** guard：同 session 已有未完成 task 时，新请求直接回 `e2a.error`（"a request is already in progress"），防并发写同一 SessionStore。`approval.respond` 与 session RPC 在此 guard **之前**内联处理（它们不产生新 run_stream task）。`finally` 块取消所有 active task + `APPROVAL_REGISTRY.cancel_all()`（断连时清理挂起 Future，防泄漏）。
...
// approval.ask — 工具审批请求（Phase 4，agent 挂起等人类决策）
{
  "type": "event",
  "event": "approval.ask",
  "payload": {
...
- `approval.ask` — 危险工具触发审批时推送（Phase 4），浏览器弹审批卡等用户 `allow_once` / `allow_always` / `deny`，通过 `approval.respond` req 回传
...
| `response_kind` | str | `e2a.chunk` / `e2a.complete` / `e2a.error` / `e2a.todo_update` / `e2a.result` / `e2a.ask` |
| `body` | dict | 载荷内容 |
| `is_stream` | bool | 固定 `true`（Twinkle 流式专用） |

**五种 response_kind**（chunk/complete/error/todo_update/result/ask）：
...
| `e2a.result` | 单帧 RPC 结果（session/history/approval.respond ack） | `{type: "session.*"|"history.get"|"approval.respond", ...}` |
| `e2a.ask` | 工具审批请求（Phase 4，is_final=false，挂起 run_stream） | `{approval_id, tool, args, tool_call_id, reason}` |
...
| `approval.ask` | 工具审批请求（Phase 4，映射 e2a.ask） |
| `result` | 单帧 RPC 结果（session/history/approval.respond ack，映射 e2a.result） |
...
twinkle/
  ...
  agentserver/
    ...
    permissions/           # 工具权限 / 审批切面（Phase 4，opt-in，默认关）
      ...
      models.py               # PermissionLevel / PermissionDecision / ToolPermissionLogEntry
      builtin_rules.py        # COMMAND_DENY_PATTERNS 单一真源（8+9=17 条）+ matches()
      policy.py               # PermissionPolicy：override→deny→tier→default 合并 + allow_always 持久化
      audit.py                # ToolPermissionLog：结构化 JSONL（fail-soft）
      approval_registry.py    # ApprovalRegistry：approval_id→Future 单例 + handle_respond
      engine.py               # PermissionEngine：通道门 + 审计 + 委托 policy
    tools/
    ...
    hooks/
      ...
      decorator.py           # @hook 装饰器（before/after_tool_call 短路 + 重试）
      builtin/
        logging_hook.py        # before/after_tool_call 日志
        permission_hook.py     # before_tool_call 权限拦截（ALLOW/DENY/ASK，priority=100）
...
tests/
  ...
  # --- Phase 3: hooks + context compression ---
  test_hook_base.py          # AgentHook / HookContext / HookInterrupt 单测
  test_hook_context.py       # HookContext extra/force_finish 单测
  test_hook_decorator.py     # @hook 装饰器短路 + 重试单测
  test_hook_manager.py       # HookManager 生命周期分发单测
  test_hook_manager_propagates_interrupt.py # HookInterrupt 传播单测
  test_agent_loop_with_hooks.py # AgentLoop + hooks 集成
  # --- Phase 4: permissions ---
  test_permissions_models.py       # PermissionLevel/Decision/LogEntry
  test_permissions_builtin_rules.py # COMMAND_DENY_PATTERNS 17 条
  ...
  test_permissions_e2e.py         # 端到端审批流（gateway + agentserver）
scripts/
