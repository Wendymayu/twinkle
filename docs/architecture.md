...
Gateway 由四个组件组成，在 `__main__.py` 中装配：
...
```
                 浏览器 ws 连入
                      ...
                      ▼
              ┌── WebChannel ──┐
              │  ws server      │  收 req → 建 Message → 回 ACK
              │  :19000         │  收 event → 广播给所有浏览器
              └───┬────────────┘
                  │ on_message 回调
                  ▼
          ┌── ChannelManager ──┐
          │  注册 + 路由        │  把 Message 转给 MessageHandler
          │  出站 dispatch 循环 │  把 robot Message 转给对应 Channel
          └───┬────────────────┘
              │
              ▼
        ┌── MessageHandler ───┐
        │  入站格式转换         │  Message → E2AEnvelope
        │  出站流式扇出         │  E2AResponse → Message(chat.delta/final)
        └───┬─────────────────┘
            │
            ▼
        ┌── AgentClient ──────┐
        │  ws client            │  连 AgentServer(:18000)
        │  demux by request_id │  按 request_id 分投 asyncio.Queue
        └──────────────────────┘
```
...
### 3.2 ChannelManager — 出站路由

[channel_manager.py](../twinkle/gateway/channel_manager.py) 是一个简单但关键的中间层：
...
**出站**：
- `publish_robot_message(msg)` — 把 Agent 响应的 `Message` 投进 `_outbound` Queue
- `_dispatch_loop` — 从 Queue 取出每条 Message，查 `channel_id` 找到对应 Channel，调 `channel.send()`

这是**单线程异步 dispatch**，不是多线程，所以没有锁。Queue + asyncio task 的模式简单可靠。

### 3.3 MessageHandler — 格式转换 + 流式扇出

[message_handler.py](../twinkle/gateway/message_handler.py) 是两种格式的翻译层：
...
**出站转换**（E2AResponse → Message）：
- 每个 chunk → `Message(event_type=chat.delta, content=chunk_content)`
- 终止帧 → `Message(event_type=chat.final, content=final_content)`
- 错误 → `Message(event_type=chat.final, content="[error] ...")`
...
出站路由：`ChannelManager._dispatch_loop` 从 Queue 取 Message，按 `msg.channel_id` 查 `_channels` dict，调对应 channel 的 `send()`。
...
⑨ MessageHandler._process_stream
   ...
   → channel_manager.publish_robot_message(msg)
                      │
                      ▼
⑩ ChannelManager._dispatch_loop
   → 取出 Message → 查 channel_id="web" → WebChannel.send(msg)
                      │
                      ▼
