...
- **`agent_loop.py`** — the ReAct core. `run_stream` is an **async generator** yielding `E2AResponse` with zero ws dependency (so it's unit-testable without sockets). Loop: `store.append(user)` → `llm.stream(msgs, tools)` → `TextDelta` yields `e2a.chunk`, `ToolCalls` get executed and the result is appended as `{role:"tool", tool_call_id, content}` then re-queried, `Done(stop)` yields `e2a.complete`. Guarded by `max_steps=8` → `e2a.error` if it doesn't converge. **Tool-result re-injection is the linchpin** — the result goes back into `SessionStore` so the next `get_messages` carries it.
