...
省一次 todo_list round-trip)。TodoStore 的 create/complete 是纯写
(返回 None),工具层在 mutation 后调用 list_tasks() 取全量再拼
markdown + snapshot。业务错误 catch 成 "Error: ..." 字符串返回。
...
async def todo_create(tasks: list[str]) -> str:
    ...
    try:
        await _todo_store.create(session_id, tasks)
        current = await _todo_store.list_tasks(session_id)
...
async def todo_complete(idx: int, result: str = "") -> str:
    ...
    session_id = get_plan_todo_session_id()
    try:
        await _todo_store.complete(session_id, idx, result)
        current = await _todo_store.list_tasks(session_id)
        append_todo_event(_snapshot(current))
...
async def todo_list() -> str:
    ...
    """
    session_id = get_plan_todo_session_id()
    tasks = await _todo_store.list_tasks(session_id)
